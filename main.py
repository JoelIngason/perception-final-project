...
def main(config_path: str) -> None:
    ...
    # Determine if rectified data is used
    use_rectified_data = config["processing"].get("use_rectified_data", True)

    # Initialize StereoCalibrator if necessary
    if not use_rectified_data:
        ...
        # Load precomputed calibration parameters for rectified data
        ...
        calib_params = calibrator.calibrate(calibration_file)
        logger.info("Calibration parameters loaded successfully")
    ...
    if not use_rectified_data:
        ...
        logger.info("Rectifier initialized.")
        ...
        for seq_id, frames in data.items():
            ...
            for frame in frames:
                try:
                    ...
                    detections, tracked_objects, depth_map = detector.detect_and_track(
                        frame.images,
                    )
                    ...
                    # Visualize
                    visualizer.display(frame, tracked_objects, depth_map)
