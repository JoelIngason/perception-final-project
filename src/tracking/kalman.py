...
class KalmanFilterXYZAH:
    ...
    def update(
        ...
        """
        # Constrain velocity in height and aspect ratio
        if measurement_mask is not None:
            if not measurement_mask[3]:  # Aspect ratio a
                mean[8] = 0  # va
                covariance[8, 8] = 1e-5
            if not measurement_mask[4]:  # Height h
                mean[9] = 0  # vh
                covariance[9, 9] = 1e-5
...
class KalmanFilterXYZWH(KalmanFilterXYZAH):
    """
    A KalmanFilterXYZWH class for tracking bounding boxes in 3D image space using a Kalman filter.

    Implements a Kalman filter for tracking bounding boxes with state space
    (x, y, z, w, h, vx, vy, vz, vw, vh), where (x, y, z) is the center position,
    w is the width, h is the height, and vx, vy, vz, vw, vh are their respective velocities.
    The object motion follows a constant velocity model, and the bounding box location
    (x, y, z, w, h) is taken as a direct observation of the state space (linear observation model).
    ...
    Examples:
        ...
        >>> kf = KalmanFilterXYZWH()
        >>> measurement = np.array([100, 50, 30, 20, 40])
        >>> mean, covariance = kf.initiate(measurement)
        >>> print(mean)
        >>> print(covariance)
    ...
    def initiate(self, measurement: np.ndarray) -> tuple:
        ...
        Args:
            measurement (ndarray): Bounding box coordinates (x, y, z, w, h) with center position (x, y, z),
                width, and height.
        ...
        """
        mean_pos = measurement  # [x, y, z, w, h]
        mean_vel = np.zeros_like(mean_pos)  # [vx, vy, vz, vw, vh]
        mean = np.r_[mean_pos, mean_vel]  # [x, y, z, w, h, vx, vy, vz, vw, vh]
        ...
        std = [
            ...
            10 * self._std_weight_velocity * measurement[4],  # vy
            40 * self._std_weight_velocity * measurement[3],  # vz
            10 * self._std_weight_velocity * measurement[3],  # vw
            10 * self._std_weight_velocity * measurement[4],  # vh
        ]
        covariance = np.diag(np.square(std))
        return mean, covariance
    ...
    def predict(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        ...
        std_pos = [
            ...
            self._std_weight_position * mean[4],  # y
            self._std_weight_position * mean[4],  # z
            self._std_weight_position * mean[3],  # w
            self._std_weight_position * mean[4],  # h
        ]
        std_vel = [
            self._std_weight_velocity * mean[3],  # vx
            self._std_weight_velocity * mean[4],  # vy
            self._std_weight_velocity * mean[3],  # vz
            self._std_weight_velocity * mean[3],  # vw
            self._std_weight_velocity * mean[4],  # vh
        ]
        motion_cov = np.diag(np.square(np.r_[std_pos, std_vel]))
    ...
    def multi_predict(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        ...
        Examples:
            >>> mean = np.random.rand(5, 10)  # 5 objects with 10-dimensional state vectors
            >>> covariance = np.random.rand(5, 10, 10)  # 5 objects with 10x10 covariance matrices
            >>> kf = KalmanFilterXYZWH()
    ...
    def update(
        ...
        Args:
            mean (ndarray): The predicted state's mean vector (10-dimensional).
            covariance (ndarray): The state's covariance matrix (10x10-dimensional).
            measurement (ndarray): The 5-dimensional measurement vector (x, y, z, w, h).
            measurement_mask (ndarray): A boolean array indicating which measurements are available.
        ...
        S = (
            covariance_proj
            + np.diag(
                [self._std_weight_position * mean[3] if i < 3 else 1e-2 for i in available_indices]
            )
            ** 2
        )
        K = np.dot(covariance, H.T) @ np.linalg.inv(S)
